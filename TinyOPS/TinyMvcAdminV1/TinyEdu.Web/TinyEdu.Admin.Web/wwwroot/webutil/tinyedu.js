// 添加到页面的window对象上面，在页面中用tinyedu.进行访问
; window.tinyedu = {};
(function ($, tinyedu) {
    "use strict";
    $.extend(tinyedu, {
        ...
        msgSuccess: function (content) {
            if (tinyedu.isNullOrEmpty(content)) {
                content = "操作成功";
            }
        ...
        checkRowEdit: function (row) {
            if (row.length == 0) {
                tinyedu.msgError("您没有选择任何行！");
            } else if (row.length > 1) {
                tinyedu.msgError("您的选择大于1行！");
            } else if (row.length == 1) {
                return true;
            }
            return false;
        ...
        ajax: function (option) {
            var opt = $.extend({
                ...
                beforeSend: function (xhr) {
                    tinyedu.showLoading("正在处理中...");
                },
                complete: function (xhr, status) {
                    tinyedu.closeLoading();
                }
            }, option);

            if (tinyedu.isNullOrEmpty(opt.url)) {
                tinyedu.alertError("url 参数不能为空");
                return;
            }
            $.ajax({
                url: opt.url,
        ...
        ajaxUploadFile: function (option) {
            ...
            if (tinyedu.isNullOrEmpty(opt.data)) {
                tinyedu.alertError("data 参数不能为空");
                return;
            }
            $.ajax({
                url: opt.url,
        ...
        exportExcel: function (url, postData) {
            tinyedu.ajax({
                ...
                beforeSend: function (xhr) {
                    tinyedu.showLoading("正在导出数据，请稍后...");
                }
            });
        },
